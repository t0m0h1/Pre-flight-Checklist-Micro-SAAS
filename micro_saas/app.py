...
def account():
    ...
    if form.validate_on_submit():
        ...
        if f:
            ...
            f.save(save_path)
            # Optionally validate image size etc.
...
def generate_pdf(checklist: Checklist, user: User):
    ...
    footer = f"Generated {datetime.utcnow().strftime('%Y-%m-%d %H:%M UTC')} • Powered by your-brand"
...
# Stripe minimal checkout (test)
import stripe
stripe.api_key = os.getenv("STRIPE_SECRET_KEY", "")

@app.route("/checkout/<int:checklist_id>", methods=["GET","POST"])
@login_required
def checkout(checklist_id):
    cl = session.query(Checklist).get(checklist_id)
    if not cl or cl.user_id != current_user.id:
        abort(404)
    # Simple price in pence (e.g., £1.50)
    price_pence = 150
    if request.method == "POST":
        try:
            ...
            intent = stripe.PaymentIntent.create(
                amount=price_pence,
                currency="gbp",
                metadata={"checklist_id": str(cl.id), "user_id": str(current_user.id)}
            )
            return {"client_secret": intent.client_secret}
        except Exception as e:
            return {"error": str(e)}, 400
    # show checkout page with publishable key
    return render_template("checkout.html", checklist=cl, stripe_publishable=os.getenv("STRIPE_PUBLISHABLE_KEY",""))
